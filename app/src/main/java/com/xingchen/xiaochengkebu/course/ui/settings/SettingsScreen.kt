...
import com.xingchen.xiaochengkebu.BuildConfig
import com.xingchen.xiaochengkebu.course.data.backup.BackupManager
import com.xingchen.xiaochengkebu.course.data.backup.BackupPayload
...
fun SettingsScreen(
    ...
    val exportJson = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("application/json")) { uri: Uri? -> uri?.let { selected -> scope.launch { runCatching { backupManager.exportJson(selected, state.records, state.semesters, state.presets) }.onFailure { errorMessage = "导出备份失败" } } } }
    val exportCsv = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("text/csv")) { uri: Uri? -> uri?.let { selected -> scope.launch { runCatching { backupManager.exportCsv(selected, state.records, state.semesters) }.onFailure { errorMessage = "导出 CSV 失败" } } } }
    val importJson = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri: Uri? -> uri?.let { selected -> scope.launch { runCatching { backupManager.importJson(selected) }.onSuccess { importPayload = it }.onFailure { errorMessage = it.message ?: "备份文件格式不正确" } } } }
