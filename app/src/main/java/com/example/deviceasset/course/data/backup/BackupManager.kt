package com.example.deviceasset.course.data.backup
...
/** File-format code is kept pure so parsing/escaping can be tested without a device. */
...
class BackupManager(private val context: Context) {
    fun exportJson(uri: Uri, records: List<TeachingRecord>, semesters: List<Semester>, presets: List<QuickPreset>) {
        context.contentResolver.openOutputStream(uri)?.use { output ->
            output.write(BackupCodec.encode(records, semesters, presets).toByteArray(Charsets.UTF_8))
        } ?: error("无法写入备份文件")
    }

    fun importJson(uri: Uri): BackupPayload {
        val raw = context.contentResolver.openInputStream(uri)?.bufferedReader(Charsets.UTF_8)?.use { it.readText() }
            ?: error("无法读取备份文件")
        return BackupCodec.decode(raw)
    }

    fun exportCsv(uri: Uri, records: List<TeachingRecord>, semesters: List<Semester>) {
